...
Appli familiale partagée : planning des animations du camping (toujours **en extra**), 28 fiches sorties détaillées (photo, accès, itinéraire, horaires, points forts — consolidées avec le panneau « Explore & Discover » du camping), carte des sorties autour du camping, 17 randonnées famille vérifiées avec itinéraires étape par étape et liens de tracés (Komoot, Visorando…), favoris votés par chaque membre (classement par votes ou par type), missions (choisies dans les sorties, randos et animations du planning, ou libres — les missions liées se cochent toutes seules), suivi des **jeux de société** (jeux différents et parties jouées, quatre paliers de 10/20 jeux et 20/30 parties), météo du jour, score famille, et un **bilan daté** de toutes les activités réalisées jour par jour. Un seul front (`index.html`) + un petit serveur Node (`server.js`) qui sert la page **et** l'API `/api/sync` (fusion horodatée, persistance PostgreSQL Render).
